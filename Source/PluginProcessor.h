...
struct Fifo
{
    void  prepare(int  numChannels, int numSamples)
    {
        static_assert( std::is_same_v<T, juce::AudioBuffer<float>>,
                      "prepare(numChannels, numSamples) should only be used when Fifo is holding a juce::AudioBuffer<float>");
    ...
    void prepare(size_t numElements)
    {
        static_assert( std::is_same_v<T, std::vector<float>>,
                      "prepare(numElements) should only be used when Fifo is holding a std::vector<float>");
        for( auto& buffer : buffers)
        {
            buffer.clear();
